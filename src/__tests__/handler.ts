...
export const handlers = [
    http.get(BASE_URL_WITH_PIID + '/list',
        () => (HttpResponse.json({
            id: 1, items: [
    ...
    http.get(BASE_URL_WITH_PIID + '/products',
        () => (HttpResponse.json({
            products: [
                { id: 1, name: 'prod1' },
                { id: 2, name: 'prod2' },
    ...
    http.patch(BASE_URL_WITH_PIID + '/item/:itemId/check',
        () => (HttpResponse.json({})),
    ),
    http.delete(BASE_URL_WITH_PIID + '/item/:itemId',
        () => (HttpResponse.json({})),
    ),
]