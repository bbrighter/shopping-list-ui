...
const HydrateAtoms = ({
	...
	// biome-ignore lint/suspicious/noExplicitAny: <useHydrateAtoms expects any>
	initialValues: any;
	children: ReactNode;
}) => {
...
const TestProvider = ({
	...
	// biome-ignore lint/suspicious/noExplicitAny: <HydrateAtoms uses any>
	initialValues: any;
	children: ReactNode;
}) => (
	<Provider>
...
const HomeProvider = () => {
	return (
		<TestProvider
			initialValues={[[piidAtom, "68a06340-c811-4820-bb18-fbe750f24f4a"]]}
		>
			<Toaster />
			<Home />
		</TestProvider>
...
describe("home page", () => {
	...
	it("Edit quantity", async () => {
		...
		const increaseButton = within(listItem1).getByTestId(
		...
		const decreaseButton = within(listItem1).getByTestId(
	...
	it("Create new item by name", async () => {
		...
		expect(spy).toHaveBeenCalledWith(
			"68a06340-c811-4820-bb18-fbe750f24f4a",
			2,
			{ name: "new item" },
		);
	});
	...
	it("Create existing item again shows error", async () => {
		...
		expect(
			screen.getByText("prod1 steht schon in der Liste"),
		).toBeInTheDocument();
	...
	it("error toast is shown", async () => {
		...
		const toast = (await screen.findByText(/Fehler mit Statuscode/)).closest(
			"li",
		) as HTMLElement;
