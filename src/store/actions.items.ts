...
export const checkItemAtom = atom(null, async (get, set, id: number) => {
	...
	const newItems = get(itemsAtom).map((it) =>
		it.id === id ? checkedItem : it,
	);
...
export const changeItemQuantityAtom = atom(
	null,
	async (get, set, id: number, newQuantity?: number) => {
		const piid = get(piidAtom);
		const item = get(itemAtom(id));
		const previousItems = get(itemsAtom);
...
export const putItemByNameAtom = atom(
	null,
	async (get, set, args: { name: string }) => {
		const piid = get(piidAtom);
		...
		const existingProduct = get(productsAtom).find(
			(p) => p.id === resp.resp.productId,
		);
		const newProducts = existingProduct
			? get(productsAtom).map((p) =>
					p.id === resp.resp.productId ? { ...p, archived: false } : p,
				)
			: [
					...get(productsAtom),
					{ id: resp.resp.productId, name: args.name, archived: false },
				];
...
export const postItemByIdAtom = atom(
	null,
	async (get, set, args: { id: number }) => {
		const piid = get(piidAtom);
		const resp = await apiWrapper(
		...
		const newItems = [
			{ id: resp.resp.id, productId: args.id, checked: false },
			...get(itemsAtom),
		];
		set(itemsAtom, newItems);
		set(resetPollingAtom, (v) => v + 1);
