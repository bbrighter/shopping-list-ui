...
export const updateProductsAtom = atom(
	null,
	async (get, set, id: number, args: { name?: string; archive?: boolean }) => {
		const piid = get(piidAtom);
		if (!piid) return;
		...
		const resp = await apiWrapper(api.PatchProduct(piid, id, patch), {
		...
		const next = prev.map((p) =>
			p.id === id
				? { ...p, name: name ?? p.name, archived: archive ?? p.archived }
				: p,
		);
...
export const removeProductsAtom = atom(null, async (get, set, id: number) => {
	...
	const resp = await apiWrapper(api.DeleteProduct(piid, id), {
